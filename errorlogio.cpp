...
void ErrorLogIO::ensureDefaultFilesExist()
{
    ...
    if (!QFile::exists(errPath)) {
        ...
        if (f.open(QIODevice::WriteOnly)) {
            const QByteArray data = QString::fromUtf8(
                "14:24:17|Нет связи с контроллером|#e74c3c|1\n"
                "13:09:29|Не удается установить необходимую температуру нагрева|#e74c3c|1\n"
                "12:15:00|Ошибка связи|#e74c3c|1\n"
                "11:30:22|Ошибка связи|#f1c40f|1\n"
                "10:57:00|Ошибка связи|#e74c3c|1\n"
                "10:34:06|Ошибка связи|#e74c3c|0\n"
            ).toUtf8();
            f.write(data);
            f.close();
        }
    ...
    if (!QFile::exists(archPath)) {
        ...
        if (f.open(QIODevice::WriteOnly)) {
            const QByteArray data = QString::fromUtf8(
                "14:24:17|Нет связи с контроллером\n"
                "13:09:29|Не удается установить необходимую температуру нагрева\n"
                "12:33:48|Ошибка связи\n"
                "10:57:00|Ошибка связи\n"
                "10:34:06|Ошибка связи\n"
            ).toUtf8();
            f.write(data);
            f.close();
        }
